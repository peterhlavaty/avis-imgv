...
// The vocabulary lives in `crate::command`, which nothing in `app` owns:
// the menus, the bar and the history all name these words and none of them
// should reach up into the shell for them. Re-exported here because reading a

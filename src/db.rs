...
pub static PREIMAGE_CACHE: OnceLock<Vec<PreImage>> = OnceLock::new();

// static DB_CONN: OnceLock<Mutex<Connection>> = OnceLock::new();
//
// fn get_db() -> &'static Mutex<Connection> {
//     DB_CONN.get_or_init(|| {
//         let path = Db::get_db_path().expect("Failed to open database! Check if the file is locked or missing.");
...
//     }
//
//     pub fn get_image_metadata(path: &str) -> Result<Option<Metadata>, Box<dyn Error>> {
//         let conn = Self::get_sqlite_conn()?;
//         let mut q = conn.prepare("select json(metadata), orientation, color_profile from file where path = ?1")?;
//
//         match q.query_row([path], |row| {
//             let orient_str: Option<String> = row.get(1)?;
//             let metadata = Metadata::new_from_db(
//                 row.get(0)?,
//                 Orientation::from_orientation_metadata(orient_str.as_ref()),
//                 row.get(2)?,
//             );
//             Ok(metadata)
//         }) {
...
//     }
//
//     pub fn get_sqlite_conn() -> Result<MutexGuard<'static, Connection>, Box<dyn Error>> {
//         //Maybe inefficient to compute this path every time?
//         // let path = Self::get_db_path()?;
//         //
//         // let conn = Connection::open(path)?;
//         // Ok(conn)
//         Ok(get_db().lock().unwrap())
//     }
//

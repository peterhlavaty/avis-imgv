...
/// keystroke starts a new path component.
fn append_separator(input: &mut String, suggestion: &str) {
    input.clear();
    input.push_str(suggestion);

    if !input.ends_with(MAIN_SEPARATOR_STR) {
        input.push_str(MAIN_SEPARATOR_STR);
    }
}
...
mod tests {
    ...
    #[test]
    fn a_path_becomes_a_string() {
        let rendered = string_from_path(Path::new("/tmp/photos"));
        assert_eq!(rendered.as_deref(), Some("/tmp/photos"));
    }

    #[test]
    fn suggestions_are_filtered_by_what_was_typed() {
        // A directory that cannot be read offers nothing rather than failing.
        assert!(get_suggestions("/definitely/not/here/").is_empty());
    }

    #[test]
    fn selecting_a_suggestion_ends_it_with_a_separator() {
        let mut input = String::new();
        let suggestion = format!("{MAIN_SEPARATOR_STR}tmp{MAIN_SEPARATOR_STR}photos");

        append_separator(&mut input, &suggestion);

        assert!(input.ends_with(MAIN_SEPARATOR_STR));
        assert!(input.starts_with(&suggestion));
    }
}

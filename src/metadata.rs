...
impl Orientation {
    pub fn from_orientation_metadata(orientation: Option<&String>) -> Option<Orientation> {
        match orientation {
            Some(o_str) => {
                let o = match o_str.as_str() {
...
#[derive(Default)]
pub struct Metadata {
    pub json: Option<String>,                  // Or serde_json::Value if using the serde_json crate
    pub orientation: Option<Orientation>,      // Using the Orientation enum from your previous code
...
impl Metadata {
    ...
    // pub fn new_from_db(json: String, orientation: Option<Orientation>, color_profile: Option<String>) -> Metadata {
    //     Metadata {
    //         json: Option::from(json),
    //         orientation,
    //         color_profile: color_profile.map(|v| v.clone()),
    //         metadata_map: None,
    //     }
    // }

    pub fn new_from_file(metadata_map: HashMap<String, String>) -> Metadata {

        let metadata_json = match serde_json::to_string(&metadata_map) {
            Ok(json) => Option::from(json),
            Err(e) => {
        ...
        Metadata {
            json: metadata_json,
            orientation: Orientation::from_orientation_metadata(metadata_map.get(METADATA_ORIENTATION)),
            color_profile: metadata_map.get(METADATA_PROFILE_DESCRIPTION).cloned(),
            metadata_map,
        }
    }

    // pub fn get_metadata_map(&mut self) -> &HashMap<String, String> {
    //     match &self.json {
    //         Some(json) => self.metadata_map.get_or_insert(serde_json::from_str(json.as_str()).unwrap_or_default()),
    //         None => self.metadata_map.get_or_insert(HashMap::new()),
    //     }
    //
    // }
    ...
    pub fn parse_exiftool_output(output: &Output, path: Option<&String>) {
        ...
        for image_metadata in re.split(&string_output) {
            if let Some((path, metadata)) = Self::parse_exiftool_output_str(image_metadata) {
                if let Some(json) = metadata.json {
                    metadata_to_insert.push((path, json))
                }
            }
        }
    ...
    pub fn parse_exiftool_output_str(output: &str) -> Option<(String, Metadata)> {
        ...
        let tags: HashMap<String, String> = output
            .lines()
        ...
        let metadata = Metadata::new_from_file(tags);

        Some((file_path.trim().to_string(), metadata))
    }

    pub fn get_image_metadata(path: &str) -> Option<Metadata> {
        // match Db::get_image_metadata(path) {
        //     Ok(opt) => {
